...
clear
    ...
    [[ -a $DHCLIENT6_FILE ]] && echo "${bold}${red}Attention${white} dhclient6.conf detected. Existing config will be replaced"
    echo "${bold}${red}WARNING${white} Network will restart at the end of this script so any existing connections will be dropped!${normal}"
    echo
    ...
    if [[ $? -eq 0 ]]; then
        echo "${bold}You have the following IPv6 addreses configured for ${blue}$INTERFACE${white}"
        echo "${green}${CURRENT_IPV6}${white}"
    ...
    while [[ $BLOCK_ADDR = "" ]]; do # to be replaced with regex
        read -p "${bold}${yellow}Input your IPv6 block address ${white}(e.g. ${underline}2001:bb8:3e23:200::${reset_underline}): ${blue}" BLOCK_ADDR
    done
    
    while ! [[ $BLOCK_SUBNET =~ ^[0-9]+$ ]]; do
        read -p "${bold}${yellow}Input your subnet for block ${white}(e.g. if it's /56, input ${underline}56${reset_underline}): ${blue}" BLOCK_SUBNET
    done

    while [[ $BLOCK_DUID = "" ]]; do # to be replaced with regex
        read -p "${bold}${yellow}Input your associated DUID ${white}(e.g. ${underline}00:03:00:00:34:b0:0c:47:4a:0e${reset_underline}): ${blue}" BLOCK_DUID
        echo -ne "${normal}"
    done
    ...
    if [[ $DISTRO = "Debian" ]]; then
        ...
        [[ -n `tail -n1 $INTERFACES_FILE | grep IPv6_Script` ]] && sed -i '1{:a;N;'$8'!b a};$d;N;P;D' $INTERFACES_FILE
